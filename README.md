...
매일 오전(09:00~) SASHBI OLAP 시스템에서 3개 리포트를 자동 다운로드하고,
...
> **현재 상태 (2026-06-15)**: 일별·월별 자동화 코드 완료 및 실데이터 검증 완료. 최종 사용자 배포 패키지(`B2C_고객지표_RPA.zip`) 완성. 개발자 PC GCP 에이전트 제거됨 — 최종 사용자 PC Task Scheduler 로 운영 전환.
...
[데이터 미준비 시: 알림 메일 → 30분 후 재시도 → 최대 12:00까지]
```
...
3. "설치 완료" 메시지 + 바탕화면 바로가기 생성 → 이후 매일 09:00 자동 실행
```
...
- Windows Task Scheduler 에 일별(09:00~12:00, 30분 간격 7회) + 월별(매월 1일 09:00) 작업 등록
...
09:00 → 09:30 → 10:00 → 10:30 → 11:00 → 11:30 → 12:00
  첫 시도부터 3시간 경과 시 포기 → 실패 메일(1회) → 수동 확인
```
...
```
매월 1일 09:00 (Task Scheduler 1회 발화)
  코드상 포기 조건: 첫 시도 기준 4일(MONTHLY_MAX_DAYS) 경과
```

> ⚠️ **알려진 제약**: `should_give_up()` 은 4일간 재시도를 허용하지만,
> 현재 `setup_user.bat` 은 월별 작업을 `/sc monthly /d 1` 로 **1일에 1회만**
> 등록합니다. 1일에 데이터가 미준비면 다음 달까지 자동 재시도되지 않습니다.
> 2~4일 자동 재시도가 필요하면 작업을 일별 트리거(`/sc daily` + 날짜 가드)로
> 바꾸거나 1~4일용 작업 4개를 등록해야 합니다. (미해결 — 운영 판단 필요)
