...
SEL_VR_HAPPYAPP_ITEM  = (
    ...
    "li:has-text('해피앱 DAU 리포트'), span:has-text('해피앱 DAU 리포트'), "
    "a:has-text('해피앱 DAU 리포트'), bdi:has-text('해피앱 GA 리포트'), "
    "bdi:has-text('해피앱 DAU 리포트')"
)
...
def _clear_date_filter(page: Page, session: BrowserSession) -> None:
    """필터 지우기 — 실패해도 계속 진행 (필터가 없을 수도 있음)."""
    log.info("[STEP] 로그인일자 필터 삭제 (선택적 단계)")
    try:
        btn = _find_in_page_or_frames(page, SEL_VR_FILTER_SETTINGS)
        btn.wait_for(state="visible", timeout=10_000)
        btn.click()
        time.sleep(0.8)
        clear_btn = _find_in_page_or_frames(page, SEL_VR_FILTER_CLEAR)
        clear_btn.click(timeout=8_000)
        time.sleep(1.0)
        log.info("  필터 지우기 완료")
        session.snapshot(page, "vr_05_filter_cleared")
    except PwTimeout:
        log.info("  필터 설정 버튼 없음 — 필터 단계 건너뜀 (필터 미설정 상태)")
        session.snapshot(page, "vr_05_no_filter")
...
def _export_mau_excel(page: Page, dl_dir: Path, session: BrowserSession) -> Path:
    ...
    try:
        mau_widget = _find_in_page_or_frames(page, SEL_VR_MAU_WIDGET)
        mau_widget.wait_for(state="visible", timeout=15_000)
        mau_widget.hover()  # ··· 버튼이 hover 시 표시되는 경우 대비
        time.sleep(0.5)

        more_btn = _find_in_page_or_frames(page, SEL_VR_MAU_MORE_BTN)
        try:
    ...
    # 데이터 내보내기 클릭
    try:
        _find_in_page_or_frames(page, SEL_VR_EXPORT_DATA).click(timeout=8_000)
        time.sleep(0.5)
    ...
    try:
        with page.expect_download(timeout=120_000) as dl_info:
            _find_in_page_or_frames(page, SEL_VR_EXPORT_CONFIRM).click(timeout=8_000)
        dl = dl_info.value
...
def _find_in_page_or_frames(page: Page, selector: str, timeout: int = 10_000):
    ...
    for frame in page.frames[1:]:
        try:
            ...
            fel.first.wait_for(state="visible", timeout=2_000)
            return fel.first
        except Exception:
            pass

    # 찾지 못하면 메인 페이지 locator 반환 (wait_for 에서 실패 메시지 발생)
...
def _click_step(
    ...
    try:
        el = _find_in_page_or_frames(page, selector)
