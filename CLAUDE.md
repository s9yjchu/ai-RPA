...
python -m src.main monthly            # 월별 (미구현, NotImplementedError)
```
...
├── config.py         .env → 동결 dataclass (Config, OlapConfig, SheetsConfig, …).
...
├── excel_parser.py   3개 파서 (parse_member_metrics, parse_channel_metrics,
│                     parse_closing_report). 상단 상수로 컬럼명 분리.
...
└── monthly_runner.py run_monthly() — NotImplementedError stub. 구현 예정.
```
...
| `src/monthly_runner.py` | 전체 구현 | LOG REPORT + VISUAL REPORT 정보 수신 후 |
