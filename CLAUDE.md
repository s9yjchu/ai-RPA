...
모든 내부 시스템은 **SPC Hub SSO** 를 통해 인증합니다. 직접 로그인 없음.

```
1. SPC Hub 로그인 (hub.spc.co.kr) — SPCHUB_ID / SPCHUB_PW
2. "확인" 버튼 클릭 (약관/공지 동의)
3. 상단 nav "System Link" → "정보화시스템" 클릭
4. 메뉴 페이지에서 대상 시스템 링크 클릭
   - OLAP       → 별도 로그인 없음 (SSO 릴레이)
   - LOG REPORT → 별도 로그인 없음 (SSO 릴레이)
   - VISUAL REPORT → 로그인 페이지 표시 시 하단 "OLAP 계정으로 로그인" 클릭
```

- **SPC Hub**: `https://hub.spc.co.kr/ekp/view/login/userLogin`
- **OLAP**: `https://dwweb.spc.co.kr:7980/SASHBI/main.jsp` (SSO)
...
```
setup.bat                             # 최초 설치 + 설정 마법사 실행 (비개발자용)
...
├── hub_login.py           SPC Hub SSO 로그인 공통 모듈. SEL_HUB_* 상수.
│                          login_to_hub() → navigate_to_olap/log_report/visual_report().
│                          새 탭 열림 여부 자동 감지, VISUAL REPORT OLAP 로그인 처리.
...
### 0. Hub 로그인 셀렉터 (`src/hub_login.py` 상단 `SEL_HUB_*` / `SEL_NAV_*` / `SEL_MENU_*`)

모든 시스템 공통 진입점. 가장 먼저 튜닝 필요.
...
- `SEL_NAV_SYSTEM_LINK` — 상단 nav "System Link" 드롭다운
- `SEL_NAV_INFO_SYSTEM` — 드롭다운 내 "정보화시스템" 항목
- `SEL_MENU_OLAP/LOG_REPORT/VISUAL_REPORT` — 정보화시스템 메뉴 링크
